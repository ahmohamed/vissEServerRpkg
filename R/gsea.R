...
gsea = funwrapper(function(
  ...
  res <- fgsea::fgsea(pathways, genelist, nproc=1, scoreType=scoretype, minSize=minSize, maxSize=maxSize)
  res_sig = res[res$padj < pvalue & !is.na(res$padj) & !is.na(res$pathway), ]
  ...
  out = visseWrapper(siggs, -log10(gsfdr), genelist, "logFC", gset_attrs = gset_attrs, org=org, overlap_measure=overlap_measure, thresh=thresh)
